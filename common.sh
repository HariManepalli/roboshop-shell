...
schema_setup() {
  if [ "${schema_type}" == "mongo" ]; then
    ...
    mongo --host mongodb.devopsb71m.online </app/schema/${component}.js &>>${log_file}
    status_check $?
    ...
    print_head "Load Schema"
    mysql -h mysql.devopsb71m.online -uroot -p${mysql_root_password} < /app/schema/shipping.sql &>>${log_file}
    status_check $?
  fi
}
